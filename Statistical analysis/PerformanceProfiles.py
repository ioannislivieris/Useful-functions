...
# Code from: https://github.com/dmsteck/perfprof.py


def perfprof(data, linespecs=None, linewidth=1.6, thmax=None,
             thlabel=r'$\tau$', plabel=r'$P(\tau)$', tol=np.double(1e-8), digit = 4,
             legendnames=None, legendpos=0, figsize = (7, 7),
    ...
    for alt in range(n):  # for each alternative
        ...
        print('%20s solved %5.2f%% of cases with the best(least) score' %(legendnames[alt], 100*y[0]) )    
...
#         plt.hold('on')
    ...
    # create legend
    if (legendnames is None):
        plt.legend(loc=legendpos, frameon = False, fontsize=legendfontsize)
    else:
        plt.legend(legendnames, loc=legendpos, frameon = False, prop={'family': 'serif', 'size': legendfontsize})
...
def perfprof2(data, linespecs=None, linewidth=1.6, thmax=None,
             thlabel=r'$\tau$', plabel=r'$P(\tau)$', tol=np.double(1e-8), digit = 4,
             legendnames=None, legendpos=0, figsize = (7, 7),
             fontsize=18, tickfontsize=14, legendfontsize=14,
             ppfix=False, ppfixmin=np.double(1e-18),
